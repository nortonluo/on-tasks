...
function commandParserFactory(Logger, Promise, _) {
    ...
    CommandParser.prototype[ls] = function(data) {
        ...
        try {
            var alllines = data.stdout.split('\n');
            var lines = alllines.slice(1,-1);
            var parsed = _.map(lines, function(line) {
                var split = line.split(' ').filter(Boolean);
                var fileType = "file";
                if (split[0].slice(0,1)==='d'){
                   var fileType= "directory";
                }
                var fileEntry = {
                    file_type: fileType,
                    permission: { user:split[0].slice(1,4),
                                 group:split[0].slice(4,7),
                                 others:split[0].slice(7,10)
                    },
                    link_count: split[1],
                    owner: split[2],
                    group: split[3],
